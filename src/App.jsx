...
function PantallaLibro({ops, clientes, gastos, hoy}) {
  ...
  // Construir movimientos de caja del día actual
  const movimientos = [];

  // 1. Operaciones del día que impactan caja
  ...
  // Filtrar por moneda y ordenar por hora
  ...
  return (
    <div style={{padding:"16px 16px 40px",maxWidth:1000,margin:"0 auto"}}>
      ...
      <div style={{fontSize:18,fontWeight:700,color:"#e2e8f0",marginBottom:4}}>Movimientos del día — {hoy}</div>
      <div style={{fontSize:11,color:"#6b7280",marginBottom:16}}>Todo lo que impactó la caja física hoy</div>

      {/* Selector moneda */}
      <div style={{display:"flex",gap:6,marginBottom:16,flexWrap:"wrap"}}>
        {MONEDAS_LIBRO.map(m=>(
          <button key={m} onClick={()=>setMonedaLibro(m)}
            style={{padding:"6px 14px",borderRadius:6,border:"1px solid "+(monedaLibro===m?"#34d399":"#1f2937"),
      ...
      <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap"}}>
        {[
          {l:"ENTRADAS",v:totalEntradas,c:"#4ade80"},
          {l:"SALIDAS",v:Math.abs(totalSalidas),c:"#f87171"},
          {l:"SALDO NETO",v:totalEntradas+totalSalidas,c:"#34d399"},
          {l:"MOVIMIENTOS",v:conSaldo.length,c:"#94a3b8",isCount:true},
        ].map(({l,v,c,isCount})=>(
          <div key={l} style={{flex:"1 1 140px",background:"#0f1623",border:`1px solid ${c}22`,borderRadius:10,padding:"12px 14px"}}>
            ...
            <div style={{fontSize:16,fontWeight:700,color:c,fontFamily:"monospace"}}>
              {isCount?v:fmt(Math.round(v))+" "+monedaLibro}
            </div>
          </div>
        ))}
      </div>
      ...
      {/* Tabla */}
      {conSaldo.length===0?(
        <div style={{textAlign:"center",color:"#374151",padding:40,fontSize:13,background:"#0f1623",borderRadius:12,border:"1px solid #1f2937"}}>
          Sin movimientos en {monedaLibro} hoy
        </div>
      ):(
        <div style={{background:"#0f1623",border:"1px solid #1f2937",borderRadius:12,overflow:"hidden"}}>
          <div style={{display:"grid",gridTemplateColumns:"70px 120px 1fr 130px 60px 130px"}}>
            {["Hora","Tipo","Concepto / Cliente","Monto","","Saldo"].map(h=>(
              <div key={h} style={{padding:"8px 10px",fontSize:9,color:"#6b7280",fontWeight:700,letterSpacing:1,borderBottom:"1px solid #1f2937",background:"#080d14"}}>{h}</div>
            ))}
            {conSaldo.map((m,i)=>(
              <Fragment key={i}>
                <div style={{padding:"10px",fontSize:11,color:"#6b7280",borderBottom:"1px solid #0a0a0a"}}>{m.hora}</div>
                <div style={{padding:"10px",borderBottom:"1px solid #0a0a0a"}}>
                  <span style={{fontSize:9,padding:"2px 7px",borderRadius:4,background:`${colorTipo[m.tipo]||"#94a3b8"}18`,color:colorTipo[m.tipo]||"#94a3b8",fontWeight:700}}>
                    {m.tipo}
                  </span>
                </div>
                <div style={{padding:"10px",fontSize:12,color:"#e2e8f0",borderBottom:"1px solid #0a0a0a"}}>
                  {m.concepto}
                  {m.nombre&&<span style={{color:"#9ca3af",marginLeft:6,fontSize:11}}>· {m.nombre}</span>}
                </div>
                <div style={{padding:"10px",fontFamily:"monospace",fontSize:13,fontWeight:700,
                  color:m.monto>=0?"#4ade80":"#f87171",textAlign:"right",borderBottom:"1px solid #0a0a0a"}}>
                  {m.monto>=0?"+":"-"}{fmt(Math.round(Math.abs(m.monto)))}
                </div>
                <div style={{padding:"10px",fontSize:10,color:"#6b7280",textAlign:"center",borderBottom:"1px solid #0a0a0a"}}>
                  {m.monto>=0?"↑":"↓"}
                </div>
                <div style={{padding:"10px",fontFamily:"monospace",fontSize:12,
                  color:m.saldoAcum>=0?"#94a3b8":"#f87171",textAlign:"right",borderBottom:"1px solid #0a0a0a"}}>
...
function FormOp({ onGuardar, onCancelar, fechaDefault, titulo, color="#fb923c", opInicial }) {
  const [f, setF] = useState({
    ...
    swapMoneda:"USDT", swapDir:"vendo", swapCantidad:"", swapTCDolar:"", swapTCMoneda:"", swapMontoARS:"", swapCliente:"", swapDestinoDif:false, swapDestinoId:"", swapDestinoBuscar:"", swapImpactaCaja:true,
  });
    ...
    <div style={{background:"#0d0d0d",border:"1px solid "+color+"33",borderRadius:10,padding:16}}>
      ...
      {f.tipo==="swap"&&(()=>{
        ...
        return (
          <div>
            ...
            <div style={{marginTop:10,marginBottom:10}}>
              <Lbl>{f.swapDir==="vendo"?"Entrego "+(f.swapMoneda||"USDT")+" desde":"Recibo "+(f.swapMoneda||"USDT")+" en"}</Lbl>
              <div style={{display:"flex",gap:8}}>
                {[{v:true,l:"💵 Caja física"},{v:false,l:"🔄 CC del cliente"}].map(opt=>(
                  <button key={String(opt.v)} type="button" onClick={()=>sf("swapImpactaCaja",opt.v)}
                    style={{flex:1,padding:"7px",borderRadius:6,
                      border:"1px solid "+((f.swapImpactaCaja!==false)===opt.v?"#06b6d4":"#1f2937"),
                      background:(f.swapImpactaCaja!==false)===opt.v?"rgba(6,182,212,0.1)":"transparent",
                      color:(f.swapImpactaCaja!==false)===opt.v?"#06b6d4":"#6b7280",
        ...
        <div style={{fontSize:10,color:"#6b7280",letterSpacing:1,marginBottom:6}}>DESGLOSE — QUIÉN ENVÍA LOS PESOS</div>
            ...
            {/* Destino diferente */}
            <div style={{marginTop:10,padding:"8px 10px",background:"rgba(6,182,212,0.04)",border:"1px solid rgba(6,182,212,0.15)",borderRadius:8}}>
              <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:12,color:"#94a3b8"}}>
                <input type="checkbox" checked={f.swapDestinoDif||false} onChange={e=>sf("swapDestinoDif",e.target.checked)}
                  style={{width:14,height:14,cursor:"pointer"}}/>
                El {f.swapMoneda||"USDT"} va a una CC distinta de quien paga
              </label>
              {f.swapDestinoDif&&(()=>{
                const clDest=clientes.find(x=>x.id===Number(f.swapDestinoId));
                const filtDest=clientes.filter(x=>!x.oculto&&(x.nombre+" "+(x.apellido||"")).toLowerCase().includes((f.swapDestinoBuscar||"").toLowerCase())).slice(0,8);
                return (
                  <div style={{marginTop:8,position:"relative"}}>
                    <Lbl>CC que recibe {f.swapMoneda||"USDT"}</Lbl>
                    {clDest&&!f.swapDestinoBuscar?(
                      <div style={{display:"flex",gap:6,alignItems:"center"}}>
                        <div style={{flex:1,padding:"6px 10px",background:"rgba(6,182,212,0.1)",border:"1px solid #06b6d444",borderRadius:6,fontSize:12,color:"#06b6d4",fontWeight:600}}>
                          {clDest.nombre} {clDest.apellido||""}
                        </div>
                        <button type="button" onClick={()=>{sf("swapDestinoId","");sf("swapDestinoBuscar","");}}
                      ...
                      <input placeholder={"Buscar CC destino "+(f.swapMoneda||"USDT")+"..."} value={f.swapDestinoBuscar||""}
                        onChange={e=>sf("swapDestinoBuscar",e.target.value)}
                        style={{width:"100%",background:"#0a0a0a",border:"1px solid #1f2937",borderRadius:6,padding:"6px 10px",color:"#e2e8f0",fontFamily:"inherit",fontSize:12,outline:"none"}}/>
                    )}
                    {f.swapDestinoBuscar&&filtDest.length>0&&(
                      <div style={{position:"absolute",left:0,right:0,background:"#111",border:"1px solid #1f2937",borderRadius:6,zIndex:200,maxHeight:140,overflowY:"auto",marginTop:2}}>
                        {filtDest.map(cl=>(
                          <div key={cl.id} onClick={()=>{sf("swapDestinoId",String(cl.id));sf("swapDestinoBuscar","");}}
                            style={{padding:"8px 12px",cursor:"pointer",fontSize:12,color:"#e2e8f0",borderBottom:"1px solid #1a1a1a"}}>
                            {cl.nombre} {cl.apellido||""}
                          </div>
                        ))}
...
function SwapForm({form, setF, clientes, swapDesglose, setSwapDesglose, fmt, parse, S, Lbl, Inp}) {
  const SWAP_MONEDAS = [{v:"USDT",l:"🟡 USDT"},{v:"EUR",l:"💶 EUR"},{v:"GBP",l:"🇬🇧 GBP"}];
  const cant = parse(form.swapCantidad||0);
  const tcD = parse(form.swapTCDolar||0);
  const tcM = parse(form.swapTCMoneda||0);
  const totalARS = parse(form.swapMontoARS||0) || cant * tcD * tcM;
  const usdBruto = tcD>0&&totalARS>0 ? Math.round(totalARS/tcD*100)/100 : Math.round(cant*tcM*100)/100;
  const totalDesgl = swapDesglose.reduce((s,d)=>s+parse(d.monto||0),0);
  const diferenciaARS = totalARS - totalDesgl;
  const gananciaUSD = form.swapDir==="vendo" ? (usdBruto - cant) : (cant - usdBruto);
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:10}}>
        {SWAP_MONEDAS.map(m=>(
          <button key={m.v} type="button" onClick={()=>setF("swapMoneda",m.v)}
            style={{flex:1,padding:"7px",borderRadius:6,border:"1px solid "+((form.swapMoneda||"USDT")===m.v?"#06b6d4":"#1f2937"),
              background:(form.swapMoneda||"USDT")===m.v?"rgba(6,182,212,0.1)":"transparent",
              color:(form.swapMoneda||"USDT")===m.v?"#06b6d4":"#6b7280",cursor:"pointer",fontFamily:"inherit",fontSize:11,fontWeight:700}}>
            {m.l}
          </button>
        ))}
      </div>
      {/* Dirección */}
      <div style={{display:"flex",gap:8,marginBottom:10}}>
        {[{v:"vendo",l:"📤 Vendo "+(form.swapMoneda||"USDT")},{v:"compro",l:"📥 Compro "+(form.swapMoneda||"USDT")}].map(d=>(
          <button key={d.v} type="button" onClick={()=>setF("swapDir",d.v)}
            style={{flex:1,padding:"7px",borderRadius:6,border:"1px solid "+((form.swapDir||"vendo")===d.v?"#06b6d4":"#1f2937"),
              background:(form.swapDir||"vendo")===d.v?"rgba(6,182,212,0.1)":"transparent",
              color:(form.swapDir||"vendo")===d.v?"#06b6d4":"#6b7280",cursor:"pointer",fontFamily:"inherit",fontSize:11,fontWeight:700}}>
            {d.l}
          </button>
        ))}
      </div>
      ...
      <div style={{marginTop:8}}>
        ...
        <Inp placeholder="Nombre del cliente" value={form.swapCliente||""} onChange={e=>setF("swapCliente",e.target.value)}/>
      </div>
      {/* Preview */}
      {cant>0&&tcD>0&&tcM>0&&(
        <div style={{marginTop:8,background:"rgba(6,182,212,0.06)",border:"1px solid rgba(6,182,212,0.2)",borderRadius:8,padding:10,fontSize:11}}>
          <div style={{...S.grid("1fr 1fr 1fr 1fr",6),marginBottom:4}}>
            <div><div style={{color:"#6b7280",marginBottom:2}}>ARS totales</div><div style={{color:"#e2e8f0",fontWeight:700}}>${fmt(Math.round(totalARS))}</div></div>
            <div><div style={{color:"#6b7280",marginBottom:2}}>USD (leg 1)</div><div style={{color:"#60a5fa",fontWeight:700}}>{usdBruto} USD</div></div>
            <div><div style={{color:"#6b7280",marginBottom:2}}>{form.swapMoneda||"USDT"} (leg 2)</div><div style={{color:"#06b6d4",fontWeight:700}}>{cant}</div></div>
            <div><div style={{color:"#6b7280",marginBottom:2}}>Ganancia</div><div style={{color:"#4ade80",fontWeight:700}}>{fmt(Math.round(gananciaUSD*100)/100)} USD</div></div>
          </div>
        </div>
      )}
      ...
      <div style={{marginTop:10}}>
        ...
        {swapDesglose.some(d=>d.monto)&&(
          <div style={{marginTop:6,fontSize:11,display:"flex",gap:12}}>
            <span style={{color:"#6b7280"}}>Distribuido: <b style={{color:"#e2e8f0"}}>${fmt(Math.round(totalDesgl))}</b></span>
            <span style={{color:Math.abs(diferenciaARS)<100?"#4ade80":"#f87171"}}>
              {diferenciaARS>100?"FALTA":"SOBRA"} ${fmt(Math.round(Math.abs(diferenciaARS)))}
            </span>
          </div>
        )}
      </div>
...
function AppInterna({ usuario }) {
  ...
  const [form, setForm] = useState({ tipo:"compra", moneda:"USD", monto:"", moneda2:"ARS", monto2:"", cotizacion:"", cliente:"", nota:"", cn:"", cpct:"", dn:"", dtm:"58", dtg:"2.5", dfr:hoy, dfv:"", dfa:"", tn:"", tpct:"", tcomFijo:"", tmoneda:"ARS", tpctOrigen:"", tpctDestino:"", ccOrigenBuscar:"", ccDestinoBuscar:"", baseImpactaCaja:"si", pagoCheqDif:"caja", pagoCheqDifCCId:"", pagoCheqDifCCBuscar:"", tipoCheqDif:"echeq", dTnaDeseada:"", swapMoneda:"USDT", swapDir:"vendo", swapCantidad:"", swapTCDolar:"", swapTCMoneda:"", swapMontoARS:"", swapCliente:"", swapDestinoDif:false, swapDestinoId:"", swapDestinoBuscar:"", swapImpactaCaja:true });
  const [swapDesglose, setSwapDesglose] = useState([{id:1,clienteId:"",buscar:"",monto:""}]);
  ...
  async function registrarOp() {
    ...
    if(tipo==="swap"){
      ...
      if(swapImpactaCaja){
        if(vendo){
          ns[form.swapMoneda] = (ns[form.swapMoneda]||0) - cant; // sale moneda de caja
        } else {
          ns[form.swapMoneda] = (ns[form.swapMoneda]||0) + cant; // entra moneda a caja
        }
      }
              ...
              <Card>
                ...
                {form.tipo==="swap"&&(
                  <SwapForm
                    form={form} setF={setF}
                    clientes={clientes}
                    swapDesglose={swapDesglose} setSwapDesglose={setSwapDesglose}
                    fmt={fmt} parse={parse} S={S} Lbl={Lbl} Inp={Inp}
                  />
                )}
